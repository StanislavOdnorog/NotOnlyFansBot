...
async def reg_name(message: types.Message, state: FSMContext):
    async with lock:
        ...
        if message.text.lower() in db['Model'].to_list():
            ...
            await bot.send_message(message.from_user.id, "Модель не найдена. Попробуй снова", reply_markup=keyboard)
            await state.finish()
            return
        ...
        await bot.send_photo(message.from_user.id, photo=img, caption=new_message, reply_markup=keyboard)
